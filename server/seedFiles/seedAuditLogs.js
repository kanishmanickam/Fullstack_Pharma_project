  ...
  try {
    ...
    for (const bill of bills) {
      ...
      entries.push({
        ...
        statusCode: 201,
        timestamp: bill.createdAt || new Date(),
      });
    }
    ...
    for (const sup of suppliers) {
      entries.push({
        ...
        details: {
          ...
          contact: sup.contact_info?.phone || "N/A",
        },
        ipAddress: ip(),
        httpMethod: "POST",
        endpoint: "SEED/api/suppliers",
    ...
    for (let i = 0; i < 60; i++) {
      ...
      entries.push({
        ...
        ipAddress: ip(),
        httpMethod: "PUT",
        endpoint: `SEED/api/inventory/${med._id}/adjust`,
        statusCode: 200,
        timestamp: daysAgoDate(rand(0, 28)),
      });
    }
    ...
    for (let i = 0; i < 4; i++) {
      entries.push({
        ...
        details: {
          targetUser: staffList[0]?.username || "staff1",
          field: "isActive",
          value: true,
        },
        ipAddress: ip(),
        httpMethod: "PUT",
        endpoint: `SEED/api/auth/users/update`,
        statusCode: 200,
        timestamp: daysAgoDate(rand(2, 25)),
      });
    }
